...
export function generateInput(n: number): number[][] {
  ...
  function randomizeArr(arr: number[]): number[] {
  ...
  function createArrRandom(n: number): number[] {
    const arr: number[] = [];
    for (let i = 0; i <= n - 1; ++i) {
      arr.push(i);
    }
  ...
  function createArrOfRandomArr(n: number): number[][] {
...
export function stableMatchingOracle(makeStableMatching: StableMatcher): void {
  for (let i = 0; i < NUM_TESTS; ++i) {
    ...
    function retInverse(arr: number[]): number[] {
      const tmp: number[] = new Array<number>(arr.length);
    ...
    const companiesInverse: number[][] = companies.map(retInverse);
    //inverse arrays of each candidate pref
    const candidatesInverse: number[][] = candidates.map(retInverse);
    ...
    for (let i = 0; i < hires.length; ++i) {
      for (let j = i + 1; j < hires.length; ++j) {
        ...
        assert(
          !(
            (a1PrefOfB2 <= a1PrefOfB1 && b2PrefOfA1 <= b2PrefOfA2) ||
            (a2PrefOfB1 <= a2PrefOfB2 && b1PrefOfA2 <= b1PrefOfA1)
          ),
